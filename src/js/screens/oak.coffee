...
module.exports = class Oak extends Screen
  ...
  load: () ->
    ...
    Async.waterfall [
      ...
      , (cb) =>
        Clash = require './../moves/clash.coffee'
        Cross = require './../moves/cross.coffee'
        Gaff = require './../moves/gaff.coffee'
        Flourish = require './../moves/flourish.coffee'

        starterMoves = [['CLASH', new Clash()],
                        ['CROSS', new Cross()],
                        ['GAFF', new Gaff()],
                        ['FLOURISH', new Flourish()]]

        @game.dialog.arrMenu 'Choose a technique to learn:', starterMoves, cb
      ...
      , (move, cb) =>
        ...
        @game.opponent.say move.name + '? I can definitely explain that... (one hour later)...
                           ...
                           that the AMERICAN WAY is best. Good luck ' + @game.player.name + '.'
      , (cb) =>
        @game.next()
      ]
